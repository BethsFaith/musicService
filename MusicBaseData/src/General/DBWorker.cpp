...
void DBWorker::update(std::string nameTable, std::string nameParam, std::string value, std::string nameId, int id,
                      std::string nameId2, int id2) {
    Poco::Data::Statement update(*m_session);
    update << Poco::format(R"(UPDATE %s SET %s='%s' WHERE %s='%d' AND %s = '%d')", nameTable, nameParam, value, nameId,
                           id, nameId2, id2);
    update.execute();
}
...
void
DBWorker::markForDeletion(std::string nameTable, std::string nameParam, std::string nameId, int id, std::string nameId2,
                          int id2) {
    Poco::Data::Statement update(*m_session);
    update << Poco::format(R"(UPDATE %s SET %s = null WHERE %s='%d' AND %s='%d')", nameTable, nameParam, nameId, id,
                           nameId2, id2);
    update.execute();
}
