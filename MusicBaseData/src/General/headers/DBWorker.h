...
public:
    ...
    template<typename T>
    void select(std::string nameParam, std::string nameTable, std::string nameParamForSearch, std::string value,
                T &container);
    ...
    template<typename T>
    bool checkValue(std::string nameParam, std::string nameTable, std::string nameParamForSearch, std::string value,
    ...
    virtual void update(std::string nameTable, std::string nameParam, std::string value, std::string nameId, int id,
                        std::string nameId2, int id2);

    virtual void markForDeletion(std::string nameTable, std::string nameParam, int id);

    virtual void
    markForDeletion(std::string nameTable, std::string nameParam, std::string nameId, int id, std::string nameId2,
                    int id2);
...
template<typename T>
void DBWorker::select(std::string nameParam, std::string nameTable, std::string nameParamForSearch, std::string value,
                      T &container) {
...
void DBWorker::select(std::string nameParam, std::string nameTable, std::string nameParamForSearch1, std::string value,
                      std::string nameParamForSearch2, std::string value2, T &container) {
    Poco::Data::Statement get(*m_session);
    get << Poco::format(R"(SELECT %s FROM %s WHERE %s='%s' AND %s = '%s')", nameParam, nameTable, nameParamForSearch1,
                        value, nameParamForSearch2, value2),
            into(container);
    get.execute();
}

template<typename T>
bool DBWorker::checkValue(std::string nameParam, std::string nameTable, std::string nameParamForSearch,
