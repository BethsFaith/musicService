...
void Album::addSong(std::string name, std::string genreName) {
    m_dataBase->insertSong(name, genreName, m_name, m_performer);
}

Album::Album(std::string name, std::string performer, Poco::SharedPtr <DataBase> dataBase) : m_name{name},
                                                                                             m_performer{performer},
                                                                                             m_dataBase{dataBase},
                                                                                             DBWorker{
                                                                                                     dataBase->getSession()
                                                                                             } {
    select("id", NAME_TABLE, "performer_id", std::to_string(m_dataBase->getPerformerIdByNickname(performer)), "name",
           name, m_id);
    if (m_id == 0)
        throw Poco::Exception("album doesn't exist");
...
std::vector <Song> Album::getSongs() {
    ...
    std::vector<int> id;
    std::vector <std::string> name, performers;
    Poco::Data::Statement get(*m_session);
    get << "SELECT songs.id, songs.name, performers.nickname as Performer FROM songs "
    ...
    for (int i{}; i < id.size(); ++i)
        songs.emplace_back(id[i], name[i], performers[i], m_name, m_dataBase);
