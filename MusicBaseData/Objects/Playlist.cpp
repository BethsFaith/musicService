...
Playlist::Playlist(const std::string &createrName, const std::string &playlistName,
                   Poco::SharedPtr<DataBase> dataBase) : m_dataBase{dataBase}, m_user{createrName}, m_name{playlistName},
...
Playlist::Playlist(const int &id, Poco::SharedPtr<DataBase> dataBase) : m_id{id}, DBWorker{dataBase->getSession()}, m_dataBase{dataBase}
{
    if (m_id == 0)
      throw Poco::Exception("playlist doesn't exist");
...
std::vector<Song> Playlist::getSongs()
{
    ...
    get << "SELECT songs.id, songs.name, performers.nickname as Performer, albums.name as Album, songs.audio FROM bond_songsplaylists "
