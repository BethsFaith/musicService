...
Song::Song(int id, std::string name, std::string performer, std::string albumName, Poco::SharedPtr <DataBase> dataBase)
        : m_id{id}, m_name{name}, m_performer{performer}, m_album{albumName}, m_dataBase{dataBase},
          DBWorker{dataBase->getSession()} {
    selectById("name", TABLES::Genres, m_id, m_genre);
}

Song::Song(std::string name, std::string performer, Poco::SharedPtr <DataBase> dataBase) : m_name{name},
...
Song::Song(int id, Poco::SharedPtr <DataBase> dataBase) : m_id{id}, m_dataBase{dataBase},
                                                          DBWorker{dataBase->getSession()} {
    if (m_id == 0)
        throw Poco::Exception("song doesn't exist");
...
bool Song::isLiked(const std::string &loginUser) {
    bool isLiked;
    select("is_liked", TABLES::LikedSongs, "song_id", std::to_string(m_id), "user_id",
           std::to_string(m_dataBase->getUserIdByLogin(loginUser)), isLiked);
    return isLiked;
}

const Poco::SharedPtr <DataBase> &Song::dataBase() const {
    return m_dataBase;
}

void Song::setDataBase(const Poco::SharedPtr <DataBase> &newDataBase) {
