...
Category::Category(const std::string &name, const std::string &coverArt, Poco::SharedPtr<DataBase> dataBase) : DBWorker{dataBase->getSession()}, m_name{name}, m_coverArt{coverArt}, m_dataBase{dataBase}
{
...
Category::Category(int id, Poco::SharedPtr<DataBase> dataBase) : DBWorker{dataBase->getSession()}, m_id{id}, m_dataBase{dataBase}
{
    if (m_id == 0)
      throw Poco::Exception("category doesn't exist");
...
void Category::addPlaylist(int playlistId)
{
    ...
    insert << Poco::format(R"(INSERT INTO %s(category_id, playlist_id) valurs (%'d','%d'))", TABLES::B_Category_Playlist, m_id, playlistId);
    insert.execute();
}
