...
User::User(const std::string nickname, Poco::SharedPtr<DataBase> dataBase) : m_nickname{nickname}, m_dataBase{dataBase}, DBWorker(dataBase->getSession())
{
...
User::User(int id, Poco::SharedPtr<DataBase> dataBase)  : m_id{id}, m_dataBase{dataBase}, DBWorker(dataBase->getSession())
{
    if (m_id == 0)
      throw Poco::Exception("user doesn't exist");
...
void User::ratePerformer(int performerId, bool rate)
{
    ...
    {
        Poco::Data::Statement insert(*m_dataBase->getSession());
        insert << "INSERT INTO likedPerformers(performer_id, user_id, is_liked, date) VALUES ($1,$2, $3, current_timestamp)",
                use(performerId),
                use(m_id),
                use(rate);
        insert.execute();
...
void User::ratePlaylist(int playlistId, bool rate)
{
    ...
    {
        Poco::Data::Statement insert(*m_dataBase->getSession());
        insert << "INSERT INTO likedPlaylists(playlist_id, user_id, is_liked, date) VALUES ($1,$2, $3, current_timestamp)",
                use(playlistId),
                use(m_id),
                use(rate);
        insert.execute();
