...
Performer::Performer(const std::string nickname, Poco::SharedPtr<DataBase> dataBase) : m_nickname{nickname}, m_dataBase{dataBase}, DBWorker(dataBase->getSession())
{
...
Performer::Performer(int id, Poco::SharedPtr<DataBase> dataBase) : m_dataBase{dataBase}, DBWorker(dataBase->getSession()), m_id{id}
{
  if (m_id == 0)
    throw Poco::Exception("performer doesn't exist");
...
std::vector<Song> Performer::getSongs()
{
    ...
    get << "SELECT songs.id, songs.name, albums.name, songs.audio FROM songs "
    ...
    for (int i{}; i < id.size(); ++i)
        songs.emplace_back(id[i], name[i], m_nickname, albums[i], audio[i], m_dataBase);
...
const Poco::SharedPtr<DataBase> &Performer::dataBase() const
{
  return m_dataBase;
}

void Performer::setDataBase(const Poco::SharedPtr<DataBase> &newDataBase)
{
