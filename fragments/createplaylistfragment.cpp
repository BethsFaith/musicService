...
void CreatePlaylistFragment::on_pb_create_clicked()
{
  ...
  m_user->m_userBase->getDataBase()->insertPlaylist(name, m_user->m_userBase->getNick(), file, ui->cb_isPublic->isChecked());
  Poco::SharedPtr<Playlist> pl = Poco::makeShared<Playlist>(m_user->m_userBase->getNick(), name, m_user->m_userBase->getDataBase());
